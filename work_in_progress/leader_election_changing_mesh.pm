 ...
 *  9) The mesh structure may change at runtime, so the number of neighbors may change at runtime
 * 10) The communication in the mesh is bidirectional, so each Node can send and receive messages from its neighbors */
...
/* rate of changing the communication indexes, adjusted to 0.2 to not occur too often */
param communication_change_rate = 0.2;
...
/* The following rule is to change the Communication indexes at runtime
 * thus lowering the constraint for static topology */
rule change_communication_indexes for i in [0,node_amount] and j in [0,node_amount] and k in [0,node_amount] {
    C[i, j]|N[k] -[ communication_change_rate ]-> C[i, k]|N[k]
}
rule change_communication_indexes_2 for i in [0,node_amount] and j in [0,node_amount] and k in [0,node_amount] and l in [0,node_amount]{
    C[i, j]|F[l, k] -[ communication_change_rate ]-> C[i, l]|F[l, k]
}
rule change_communication_indexes_3 for i in [0,node_amount] and j in [0,node_amount] and k in [0,node_amount] {
    C[i, j]|L[k] -[ communication_change_rate ]-> C[i, k]|L[k]
}
